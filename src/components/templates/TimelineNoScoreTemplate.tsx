...
const TimelineNoScoreTemplate: FC<{ data: ResumeData }> = ({ data }) => {
                    ...
                    <div className="space-y-3">
                        {skillsGroups.map((group, index) => (
                            <div key={index} className="flex items-start text-sm">
                                <div className="bg-gray-700 text-white font-semibold px-3 py-1 min-w-[100px] text-center">{group.groupName}</div>
                                <div className="flex flex-wrap gap-x-2 gap-y-1 p-2 border rounded-r-md flex-1">
                                    {group.skills.split(',').map(s => s.trim()).filter(Boolean).map((skill, i) => (
                                        <span key={i} className="bg-gray-200 px-2 py-0.5 rounded">{skill}</span>
                                    ))}
                                </div>
                            </div>
                        ))}
...
export default TimelineNoScoreTemplate;
